...
* **Session Idle Timeout:** Idle UDP flows will terminate (time out) after the specified time period. Subsequent UDP packets could be load balanced to a new server unless a <a href="/docs/16.2/configuration-guide/templates/profiles/persistence-profile/">persistence profile</a> is applied.   