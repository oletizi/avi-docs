...
* Install the Avi SDK
<pre><code class="language-lua">pip install avisdk wget https://github.com/avinetworks/ansible-modules-extras/archive/devel.tar.gz sudo mkdir -p /etc/ansible/library
tar -xvf devel.tar.gz -C /etc/ansible/library ansible-playbook -M /etc/ansible/library/ansible-modules-extras-devel/network/avi export ANSIBLE_LIBRARY=/etc/ansible/library/ansible-modules-extras-devel/network/avi ---
- hosts: localhost
  connection: local
  vars:
