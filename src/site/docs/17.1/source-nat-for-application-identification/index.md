...
Source NAT can be used with either of the high availability (HA) modes, <a href="{% vpath %}/elastic-ha-for-avi-service-engines">elastic</a> HA or <a href="{% vpath %}/legacy-ha-for-avi-service-engines/">legacy HA</a>. The configuration requirements differ depending on whether the SE and back-end servers are in the same subnet (connected at Layer 2) or in different subnets (connected at Layer 3).
...
* In Layer 3 HA, the upstream router is used to provide equal-cost multipath (<a href="{% vpath %}/legacy-ha-for-avi-service-engines/">ECMP</a>) load balancing across the virtual service's SEs.
...
For details on enabling BGP to advertise SNAT IP addresses, <a href="{% vpath %}/bgp-support-for-virtual-services/">click here</a>.
...
A floating interface IP needs to be provisioned in order to provide adjacency to the upstream router, for the SNAT IP. For information, <a href="{% vpath %}/legacy-ha-for-avi-service-engines/">click here</a>.
