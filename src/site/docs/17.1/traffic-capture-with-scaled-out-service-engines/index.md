...
To view traffic flowing through a single SE for a scaled out virtual service, add a filter to the pcap viewer with the desired SE's IP address on the server side of the connection. See <a href="{% vpath %}/manual-traffic-capture/">Traffic Capture</a> for more on this topic.    
