...
<ol> 
 ...
 <li>After the wizard closes, see the following articles to complete the installation and create virtual services: 
  <ol> 
   <li><a href="{% vpath %}/public-key-management-on-se-hosts/">Public Key Management on Service Engine Hosts</a></li>
   <li><a href="{% vpath %}/adding-avi-service-engines-to-a-linux-server-cloud">Adding Avi Service Engines to a Linux Server Cloud</a></li>
  </ol> </li> 
</ol>  
...
Note: If the wizard has timed out or you have decided to click through the rest of the wizard and do the SSH setup later, go <a href="{% vpath %}/public-key-management-on-se-hosts/">here instead</a>, when ready. See the same link if using an account other than "root". This section assumes that "root" will be used.
