...
<ol> 
 ...
 <li>Enter OpenStack settings: 
  <ul> 
   ...
   <li><strong>(NEW in 16.3) </strong>If a username "test" is created as a keystone v3 user in a domain named "default" then we need to explicitly specify "test@testdomain" while logging into Avi Controller otherwise, keystone will try just throw "invalid user/password". If the domain name is not specified then keystone looks for domain with UUID "testdomain" and not name "testdomain" and since there is no domain with UUID as "testdomain", keystone would fail.</li> 
