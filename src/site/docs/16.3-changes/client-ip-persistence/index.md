...
Since client IP persistence is stored locally on each Service Engine, larger tables will consume more memory.  For very large persist tables, consider adding additional memory to the SEs via the SE Group properties for SE memory and via the SE Group > Connection table setting. See <a href="/se-memory-consumption/">SE Memory Consumption </a>for more on this topic.
...
<pre><code class="language-lua">:&gt; show pool test-pool persistence | grep 10.1.1.1</code></pre>  
